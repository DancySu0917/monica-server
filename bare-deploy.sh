#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════
# Monica Medical AI Server — 裸机一键部署脚本
#
# 支持系统：Ubuntu 20.04 / 22.04 / Debian 11+
#
# 用法：
#   chmod +x bare-deploy.sh
#   sudo ./bare-deploy.sh install   # 首次安装（安装依赖 + 配置服务）
#   sudo ./bare-deploy.sh update    # 更新代码并重启
#   ./bare-deploy.sh logs           # 查看实时日志
#   sudo ./bare-deploy.sh stop      # 停止服务
#   sudo ./bare-deploy.sh restart   # 重启服务
#   ./bare-deploy.sh status         # 查看运行状态
#   sudo ./bare-deploy.sh uninstall # 卸载服务（保留数据）
#
# 部署后目录结构：
...
cmd_install() {
    ...
    if [ ! -f "${APP_DIR}/.env" ]; then
        ...
        echo -e "    ${YELLOW}EVOLINK_API_KEY${NC} → 填写你的 Evolink API Key"
        echo -e "    ${YELLOW}ALLOWED_ORIGINS${NC} → 填写前端域名，如 https://example.com"
        echo ""
        error "请先完成 .env 配置，然后重新运行 sudo ./bare-deploy.sh install"
    fi
    _check_env_values
    success ".env 配置检查通过"
    ...
    until curl -sf http://localhost:8000/health >/dev/null 2>&1; do
        j=$((j+1))
        [ $j -ge 18 ] && error "服务启动超时，请查看日志：./bare-deploy.sh logs"
        sleep 5; echo -n "."
    done
    echo ""
    success "应用服务已就绪"
    ...
    echo -e "  📋 查看日志  : ./bare-deploy.sh logs"
    echo -e "  📊 查看状态  : ./bare-deploy.sh status"
    echo ""
}
...
cmd_update() {
    ...
    local i=0
    until curl -sf http://localhost:8000/health >/dev/null 2>&1; do
        i=$((i+1)); [ $i -ge 18 ] && error "重启超时，请查看日志：./bare-deploy.sh logs"
        sleep 5; echo -n "."
    done
    echo ""
    success "✅ 更新完成！"
...
; ── Monica Medical AI Server - Supervisor 配置（裸机模式）──────
...
# Monica Medical AI Server - Nginx 反向代理（裸机模式）
